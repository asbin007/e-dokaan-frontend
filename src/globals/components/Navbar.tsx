...
const Navbar = () => {
  const reducxToxen = useAppSelector((store) => store.auth.user.token)
  const localStorageToken = localStorage.getItem("tokenHoYo")
  const [loggedIn, setLoggedIn] = useState<boolean>(false)

  useEffect(() => {
    if (reducxToxen && localStorageToken) {
      setLoggedIn(true)
    }
  }, [])
        ...
        <div className="hidden md:block">
          {
            loggedIn ? (
              <Link to={"/logout"}>
                <button type="button" className=" mr-5 py-3 px-8 text-sm bg-teal-500 hover:bg-teal-600 rounded text-white">
                  LogOut
                </button>
              </Link>
            ) : (
