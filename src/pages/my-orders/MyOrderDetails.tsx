...
function MyOrderDetails() {
  ...
  const {id}=useParams()
  const {}=useAppSelector((store)=>store.orders)
  useEffect(()=>{
    if(id)
                ...
                <div className="flex justify-center w-full md:justify-start items-center space-x-4 py-8 border-b border-gray-200">
                  <div className="flex justify-start items-start flex-col space-y-2">
                    <p className="text-sm dark:text-gray-300 leading-5 text-gray-600">
                      Full Name : {MyOrderDetails[0]?.Order?.firstName}{" "}
                      {MyOrderDetails[0]?.Order?.lastName}
                    </p>

                    <p className="text-sm dark:text-gray-300 leading-5 text-gray-600">
                      Phone No : {MyOrderDetails[0]?.Order?.phoneNumber}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 xl:space-y-12 md:space-y-0 md:flex-row items-center md:items-start">
                  <div className="flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8">
                    ...
                    <p className="w-48 lg:w-full dark:text-gray-300 xl:w-48 text-center md:text-left text-sm leading-5 text-gray-600">
                      {" "}
                      {MyOrderDetails[0]?.Order?.AddressLine},{" "}
                      {MyOrderDetails[0]?.Order?.City},{" "}
                      {MyOrderDetails[0]?.Order?.State}
                    </p>
                  </div>
                </div>
